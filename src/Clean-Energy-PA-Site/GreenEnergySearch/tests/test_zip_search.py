...
from web_parser.tests import zipsearch_test_data
...
class Zip_Search_View_Test(GreenEnergySearchBaseTest):
    ...
    def test_distributors_selection(self, mock_web_parser):
        """Test ID 59: Test verifies that the distributors are shown"""

        # Mock the distributors reponse
        mock_web_parser.return_value.get_distributors.return_value = distributor_collection(zipsearch_test_data.multiple_expected_example)
    ...
    @mock.patch('web_parser.papowerswitch_api.papowerswitch_api')
    def test_rateschedule_selection(self, mock_web_parser):
        ...
        # Create the zip_search url with a test zip code
        url = build_rate_type_path("15025", "27487")
        ...
        self.assertEqual(len(element.findChildren()), 3)
    
    @mock.patch('web_parser.papowerswitch_api.papowerswitch_api')
    def test_offers_results(self, mock_web_parser):
        """Test ID 61: Test verifies that the offers are shown"""

        # Mock the distributors reponse
        mock_web_parser.return_value.get_offers.return_value = offer_collection(ratesearch_test_data.expected_example)
        # self.mock_distributors_response(zipsearch_test_data.multiple_expected_example)
